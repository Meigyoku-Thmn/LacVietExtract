(() => {
    ...
    define("helpers/winapi", ["require", "exports", "helpers/frida-struct"], function (require, exports, frida_struct_1) {
        ...
        function AddVectoredExceptionHandler(First, callback) {
            const Handler = new NativeCallback(callback, 'int32', ['pointer'], 'stdcall');
        ...
        const _VirtualProtect = new SystemFunction(Kernel32.getExportByName('VirtualProtect'), 'uint32', ['pointer', 'uint32', 'uint32', 'pointer'], 'stdcall');
        function VirtualProtect(address, size, newProtect) {
            const oldProtectPtr = Memory.alloc(4);
            const result = _VirtualProtect(address, size, newProtect, oldProtectPtr);
            if (result.value == 0)
        ...
        (function trackDecompressionRoutine() {
            ...
            Interceptor.attach(func, {
                ...
                onLeave(rs) {
                    ...
                    logWriteLine("Base Address: " + buffer);
                    MemoryAccessMonitor.enable({ base: buffer, size: bufferSize }, {
                        onAccess(details) {
                            const instr = Instruction.parse(details.from);
                            const nByte = instr.operands[instr.operands.length - 1].size;
                            logWriteLine(`Memory access: ${details.operation} buffer+${details.address.sub(buffer)} from ${norm_mtdDataLIB_addr(details.from)} ${nByte} byte(s)`);
                        }
                    });
                }
            });
    ...
    define("shellcode_playground", ["require", "exports", "helpers/utils", "helpers/winapi", "helpers/frida-struct"], function (require, exports, utils_2, winapi_2, frida_struct_2) {
        "use strict";
        Object.defineProperty(exports, "__esModule", { value: true });
        (0, utils_2.sendCommand)('Ping');
        setTimeout(() => {
            const arrSize = 8;
            const arrPtr = Memory.alloc(arrSize);
            arrPtr.writeByteArray([1, 2, 3, 4, 5, 6, 7, 8]);
            const beginPageAddress = (0, winapi_2.VirtualQuery)(arrPtr).BaseAddress;
            const endPageAddress = (0, winapi_2.VirtualQuery)(arrPtr.add(arrSize - 1)).BaseAddress.add(Process.pageSize);
            let currentMemoryAddress = 0;
            const exceptionInfo = (0, frida_struct_2.createCStruct)(winapi_2.EXCEPTION_POINTERS, null);
            const exceptionRecord = (0, frida_struct_2.createCStruct)(winapi_2.EXCEPTION_RECORD, null);
            (0, winapi_2.AddVectoredExceptionHandler)(1, (exPtr) => {
                exceptionInfo.setPtr(exPtr);
                exceptionRecord.setPtr(exceptionInfo.ExceptionRecord);
                const exceptionCode = exceptionRecord.ExceptionCode;
                console.log(exceptionCode == winapi_2.EXCEPTION_GUARD_PAGE);
                if (exceptionCode == winapi_2.EXCEPTION_GUARD_PAGE) {
                    currentMemoryAddress = exceptionRecord.ExceptionInformation[1];
                    if (currentMemoryAddress < beginPageAddress.toUInt32() || currentMemoryAddress >= endPageAddress.toUInt32())
                        return winapi_2.EXCEPTION_CONTINUE_SEARCH;
                    if (currentMemoryAddress >= arrPtr.toUInt32() && currentMemoryAddress < arrPtr.add(arrSize).toUInt32()) {
                        const mode = exceptionRecord.ExceptionInformation[0];
                        const modeStr = mode == 0 ? "Read" :
                            mode == 1 ? "Write" : "Execute";
                        console.log(`${modeStr} at arr+${(currentMemoryAddress - arrPtr.toUInt32()) / 4}`);
                    }
                    // Set up for STATUS_SINGLE_STEP
                    const EFLags = exceptionInfo.ContextRecord.add(192).readU32();
                    exceptionInfo.ContextRecord.add(192).writeU32(EFLags | 0x00000100);
                    return winapi_2.EXCEPTION_CONTINUE_EXECUTION;
                }
                else if (exceptionCode == winapi_2.STATUS_SINGLE_STEP) {
                    if (currentMemoryAddress == 0)
                        return winapi_2.EXCEPTION_CONTINUE_SEARCH;
                    (0, winapi_2.VirtualProtect)(ptr(currentMemoryAddress), 1, winapi_2.PAGE_READWRITE | winapi_2.PAGE_GUARD);
            ...
            const oldProtect = (0, winapi_2.VirtualProtect)(arrPtr, arrSize, winapi_2.PAGE_READWRITE | winapi_2.PAGE_GUARD);
            const arr = new Uint8Array(ArrayBuffer.wrap(arrPtr, arrSize));
            arr[0] = arr[7];
            arr[0] = 5;
            arr[1] = 9;
            // for (let i = 0; i < arrSize; i++)
            //    console.log(arr[i]);
            // VirtualProtect(arrPtr, arrSize, oldProtect);
        }, 1000);
    });
    //# sourceMappingURL=shellcode_playground.js.map
    'marker:resolver';
